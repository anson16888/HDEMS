...
2. 《Web应用安全开发规范》
3. Ant Design Pro 官方文档：https://pro.ant.design/zh-CN
4. Gin框架官方文档：https://gin-gonic.com/docs/
5. React.js官方文档：https://react.dev/
...
| **M**easurable（可衡量的） | 支持10家医院部署，每家医院管理不超过2000条排班记录和2000条物资记录，数据采集延迟≤5秒 |
| **A**chievable（可实现的） | 采用成熟的技术栈（Golang/Gin + React.js/Ant Design Pro），使用旗云现有中台采集服务 |
...
| 角色名称 | 角色描述 | 主要职责 |
|----------|----------|----------|
| **系统管理员** | 系统最高权限管理者 | 用户管理、角色分配、系统配置、数据备份 |
| **值班管理员** | 值班管理员（支持卫健委/医院两级） | 卫健委值班管理员：管理局级行政排班、查看所有医院排班总览<br/>医院值班管理员：管理本院的行政排班和主任排班 |
| **物资管理员** | 物资管理员 | 管理本院的物资库存信息 |
| **管理人员** | 管理者（支持卫健委/医院两级） | 卫健委管理人员：查看所有医院的排班和物资统计信息<br/>医院管理人员：查看本院的排班和物资统计信息 |

**权限机制说明：**
- 系统采用**4种固定角色 + 组织层级**的组合权限设计
- 值班管理员和管理人员通过**所属组织（卫健委/医院）+ 角色类型**确定具体权限
- 例如：卫健委值班管理员 vs 医院值班管理员，虽然是同一角色，但权限范围不同

#### 2.2.2 角色权限矩阵

| 功能模块 | 系统管理员 | 值班管理员<br/>(卫健委) | 值班管理员<br/>(医院) | 物资管理员 | 管理人员<br/>(医院) | 管理人员<br/>(卫健委) |
|----------|-----------|---------------------|-----------------|---------------|---------------|-----------------|
| **用户管理** | ✅完全控制 | ❌ | ❌ | ❌ | ❌ | ❌ |
| **基础数据管理** | ✅完全控制 | ✅查看/编辑 | ✅查看/编辑 | ❌ | ❌ | ❌ |
| **医院信息管理** | ✅完全控制 | ❌ | ❌ | ❌ | ❌ | ❌ |
| **库存阈值配置** | ✅完全控制 | ❌ | ❌ | ❌ | ❌ | ❌ |
| **局级行政排班** | ✅完全控制 | ✅完全控制 | ❌ | ❌ | ✅查看 | ✅查看 |
| **院级行政排班** | ✅完全控制 | ❌ | ✅完全控制 | ❌ | ✅查看 | ✅查看 |
| **院内专家排班** | ✅完全控制 | ❌ | ✅完全控制 | ❌ | ✅查看 | ✅查看 |
| **排班一览表** | ✅查看 | ✅查看全部 | ✅查看本院 | ❌ | ✅查看本院 | ✅查看全部 |
| **物资管理** | ✅完全控制 | ❌ | ❌ | ✅完全控制 | ✅查看本院 | ✅查看全部 |
...
#### 6.1.3 交互方式
- 表单提交：前端校验 + 后端校验
- 操作反馈：成功/失败消息提示（Ant Design Message）
- 确认对话框：删除、批量操作等危险操作
- 加载状态：Loading动画
- 分页：支持页码跳转和每页条数选择
...
| 层级 | 技术 | 说明 |
|------|------|------|
| **前端框架** | React.js 18+ | UI框架 |
| **UI组件库** | Ant Design Pro 6+ | 企业级UI组件 |
| **状态管理** | Redux Toolkit | 状态管理 |
| **HTTP客户端** | Axios | API请求 |
| **后端框架** | Golang 1.19+ | 后端语言 |
| **Web框架** | Gin 1.9+ | HTTP框架 |
| **ORM框架** | GORM | 数据库ORM |

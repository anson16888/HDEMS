<template>
  ...
  >
    <a-space direction="vertical" :size="24" style="width: 100%">
      ...
      <a-alert
        message="导入说明"
        description="请先下载模板，填写排班信息后上传。单次最多导入5000条记录，文件大小不超过10MB。"
        type="info"
        show-icon
      />
...
async function handleDownloadTemplate() {
  try {
    const blob = await scheduleApi.downloadScheduleTemplate(props.scheduleType)

    // 创建下载链接
    const url = window.URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = '排班导入模板.xlsx'
    link.click()
...
async function handleImport() {
  ...
  try {
    ...
    formData.append('file', selectedFile.value.originFileObj)

    const response = await scheduleApi.importSchedules(props.scheduleType, formData)

<template>
  <div class="hospital-schedule-page">
    <!-- Page Header -->
    <a-page-header title="院级行政排班" sub-title="管理医院级别的行政排班信息，支持增删改查及Excel导入导出" />
    ...
    <ScheduleImportModal
      ...
      :schedule-type="SCHEDULE_TYPE.HOSPITAL"
...
const loadSchedules = async () => {
  try {
    ...
    const params = {
      scheduleType: SCHEDULE_TYPE.HOSPITAL,
...
const loadBasicData = async () => {
  try {
    ...
    const [deptRes, shiftRes, rankRes, titleRes, hospitalRes] = responses
...
const handleExport = async () => {
  try {
    ...
    link.href = url
    link.download = `院级行政排班_${dayjs().format('YYYYMMDD_HHmmss')}.xlsx`
    link.click()
